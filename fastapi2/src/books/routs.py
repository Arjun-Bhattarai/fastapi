...
async def get_book_by_id(book_id: UUID, session: AsyncSession = Depends(get_session), user: dict = Depends(access_token)):
...
async def create_book(book: BookCreate, session: AsyncSession = Depends(get_session), user: dict = Depends(access_token)):
    new_book = await book_service.create_book(book, session)
    return new_book

@router.put("/{book_id}", response_model=BookRead)
async def update_book(book_id: UUID, book: BookUpdate, session: AsyncSession = Depends(get_session), user: dict = Depends(access_token)):
